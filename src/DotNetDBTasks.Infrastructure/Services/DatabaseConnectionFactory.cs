    ...
    public string BuildConnectionString(DatabaseUser dbUser, string password)
    {
        return dbUser.ServerType switch
        {
            DatabaseServerType.Oracle =>
                $"Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST={dbUser.Host})(PORT={dbUser.Port}))(CONNECT_DATA=(SERVICE_NAME={dbUser.ServiceName})));User Id={dbUser.DbUsername};Password={password};",

            DatabaseServerType.SqlServer =>
                $"Server={dbUser.Host},{dbUser.Port};Database={dbUser.DatabaseName};User Id={dbUser.DbUsername};Password={password};TrustServerCertificate=True;",

            DatabaseServerType.PostgreSql =>
                $"Host={dbUser.Host};Port={dbUser.Port};Database={dbUser.DatabaseName};Username={dbUser.DbUsername};Password={password};",

            DatabaseServerType.MySql =>
                $"Server={dbUser.Host};Port={dbUser.Port};Database={dbUser.DatabaseName};User={dbUser.DbUsername};Password={password};",
