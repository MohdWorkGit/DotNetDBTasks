...
public class QueryExecutor : IQueryExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            // Non-SELECT queries (INSERT, UPDATE, DELETE, MERGE, etc.) return affected row count
            var affectedRows = await command.ExecuteNonQueryAsync(cancellationToken);
            result.AffectedRows = affectedRows;
            result.TotalRows = affectedRows;
        }
    ...
    /// <summary>
    /// Determines whether the SQL query is a SELECT statement that returns a result set.
    /// </summary>
    private static bool IsSelectQuery(string sql)
    {
        var trimmed = sql.TrimStart();
        // Handle common SELECT patterns including WITH (CTE) ... SELECT
