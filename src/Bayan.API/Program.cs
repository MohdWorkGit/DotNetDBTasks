...
using Bayan.API.BackgroundJobs;
using Bayan.API.Extensions;
...
// (sc.exe create ... binPath=Bayan.API.exe) it wires up service lifetime
...
{
    app.UseSwagger();
    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Bayan API v1"));
}
...
Log.Information("Bayan API started successfully.");
app.Run();
