...
| `POST /` | `reports.manage` | Creates a report. Dataset keys must be unique within the report and match `[A-Za-z0-9_]+`; a chart key may not collide with a dataset key, since the template's `{{RESULTS:x}}` and `{{CHART:x}}` share one namespace. Join and detail datasets are rejected if they form a cycle. `maxDetailRows` and `maxTotalRows` accept **0 for no limit** and are otherwise unbounded — only a negative value is refused, with 400. A detail dataset costs one query execution per parent row, so 0 there means the run's timeout is the only thing bounding an N+1. |
